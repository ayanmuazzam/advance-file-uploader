		...
		public function custom_payment_update_order_meta( $order_id ) {
			...
			if (!empty($cart_data)) {
				...
				foreach ($extendons_upload_file_rules as $value) {
					if ('extendons_upload_files_enable' == $value['extendons_upload_files_enable_disable_status']) {
							$rule_id = $value['extendons_post_id'];
							$checkout_page_flag = ( 'extendons_upload_file_after_checkout_page_after_notes_field' == $value['extendons_upload_files_display_on_position'][0]['extendons_allow_uf_after_checkout_page_after_notes_field'] ) 
							? true : ( ( 'extendons_upload_files_checkout_page_after_notes' == $value['extendons_upload_files_display_on_position'][0]['extendons_allow_uf_checkout_page'] ) ? true : false );
						...
						foreach ($value['extendons_upload_files_multiple_files_limit'] as $key => $file_upload_rule_settings) {
							...
							$extendons_upload_files_session = WC()->session->get($session_key, array());
						
							if (empty($extendons_upload_files_session) && 'true' == $file_upload_rule_settings['extendons_uploadfiles_file_required'] && $checkout_page_flag) {
								// Show notice if required files are not uploaded
								wc_add_notice(esc_html__('Please upload the required files for ' . $file_upload_rule_settings['extendons_uploadfiles_file_text_btn'], 'extendons_Upload_Files'), 'error');
								// Skip saving to order meta if no files are uploaded
								$extendons_upload_files_session = array();
								$customer_notes = '';
								// Continue to the next iteration
								continue;
							}
							if (!empty($extendons_upload_files_session)) {
								// Save uploaded files to order meta with a unique key per rule
								$order->update_meta_data('extendons_upload_files_' . $rule_id . '_' . $key, $extendons_upload_files_session);
							}

							if (!empty($customer_notes)) {
								// Save customer notes to order meta with a unique key per rule
								$order->update_meta_data('extendons_customer_notes_' . $rule_id . '_' . $key, $customer_notes);
							}

							// Optionally, clear the session data after saving to order meta
							WC()->session->__unset($notes_session_key);

							// Unset the session data for the rule ID
							if ( WC()->session && WC()->session->has_session() ) {
								$extendons_upload_files_session = WC()->session->get( $session_key );
								if ( ! empty( $extendons_upload_files_session ) ) {
									// Unset the session data for the rule ID
