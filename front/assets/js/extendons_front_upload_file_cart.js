jQuery(document).ready(function ($) {
  ...
  function validateFiles(files, $container, ruleId, key) {
    ...
    ruleDataMap[ruleId][key].validatedFiles = ruleDataMap[ruleId].fileChosenArray;
    return validFiles;
  }

  // Display error or success message
  function showMessage(message, type, $container, ruleId, key) {
  ...
  // Remove an image
  function removeImage(index, $imageItem, $container, ruleId, key) {
    const $counter = $container.find(
      `#extendons_upload_files_counter_${ruleId}`
    );
    const $fileInput = $container.find(".extendons_upload_files_hidden_input");
    ...
    if (!(file instanceof File) && file.url) {
      ...
      showMessage("Image removed successfully!", "success", $container, ruleId, key);
    }
  }
    ...
    $.ajax({
      ...
      success(response) {
        if (response.success) {
          displaySelectedImages($container, ruleId, key);
          showMessage(
            "Files and notes uploaded successfully.",
            "success",
            $container,
            ruleId,
            key
  ...
  $("body").on("click", ".extendons_upload_files_delete_button", function (e) {
    ...
    if (confirm("Are you sure you want to remove this image?")) {
      removeImage(index, $imageItem, $container, ruleId, key);
    }
  });
