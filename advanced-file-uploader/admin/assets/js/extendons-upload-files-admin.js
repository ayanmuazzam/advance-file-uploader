...
function extendons_file_upload_additional_settings() {
    ...
    formData.append("action", "extendons_upload_additional_settings_save");
    formData.append("extendons_upload_folder_path", jQuery("#extendons_upload_folder_path").val());
    formData.append("extendons_enable_google_drive_uploads", jQuery("#extendons_enable_google_drive_uploads").is(":checked") ? "true" : "false");
    formData.append("extendons_google_drive_folder_name", jQuery("#extendons_google_drive_folder_name").val());
    formData.append("nonce", ewcpm_php_vars.nonce);

    let jsonFileInput = jQuery("#extendons_google_drive_json_file")[0];
    if (jsonFileInput && jsonFileInput.files.length > 0) {
        formData.append("extendons_google_drive_json_file", jsonFileInput.files[0]);
    }
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            jQuery("#message_additional_settings").delay(3000).fadeOut("slow");
...
function extendons_upload_file_save_general_settings(formaction) {
    ...
    var extendons_uf_allow_modification;
    var valid = true;
    jQuery('input[name="extendonspriceuploadfilelabel[]"]').each(function () {
        ...
        if(
            jQuery(this)
                ...
                .parent()
                .parent()
                .find('input[name="extendons_uf_allow_modification[]"]')
                .prop("checked") == true
        ) {
            extendons_uf_allow_modification = "true";
        } else {
            extendons_uf_allow_modification = "false";
        }
        ...
        var temp_array = {
            ...
            extendons_uploadfiles_file_allow_modification: extendons_uf_allow_modification,
        };
